...
from .settings import FRONTEND_ASSETS_DIR, FRONTEND_DIST_DIR, FRONTEND_PUBLIC_DIR, ROOT_DIR, STATIC_DIR
...
async def serve_image(file_path: str):
    image_root = (ROOT_DIR / "images").resolve()
    file = (image_root / file_path).resolve()
    if not file.is_relative_to(image_root):
...
async def serve_dashboard_png():
    return FileResponse(FRONTEND_PUBLIC_DIR / "map.jpg")
...
async def serve_map_jpg():
    map_file = FRONTEND_PUBLIC_DIR / "map.jpg"
    if not map_file.is_file():
        raise HTTPException(status_code=404, detail="map.jpg not found")
